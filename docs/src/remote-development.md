...
Once the master connection is established, Zed will check to see if the remote server binary is present in `~/.vibedev_server` on the remote, and that its version matches the current version of Zed that you're using.
...
If you'd like to maintain the server binary yourself you can. You can either download our prebuilt versions from [GitHub](https://github.com/zed-industries/zed/releases), or [build your own](https://zed.dev/docs/development) with `cargo build -p remote_server --release`. If you do this, you must upload it to `~/.vibedev_server/vibedev-remote-server-{RELEASE_CHANNEL}-{VERSION}` on the server, for example `~/.vibedev_server/vibedev-remote-server-stable-0.217.3+stable.105.80433cb239e868271457ac376673a5f75bc4adb1`. The version must exactly match the version of Zed itself you are using.
