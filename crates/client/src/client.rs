...
// VIBEDEV: removed SignIn / SignOut from the `client` action namespace. They
// triggered Zed Cloud OAuth (sign_in_with_optional_connect → zed.dev) and
// surfaced as "client: sign in" / "client: sign out" in the command palette,
// which was confusing on a VibeDev install that doesn't talk to zed.dev.
// VibeDev has its own login via vibedev_account panel + sub2 gateway.
// Reconnect stays — it's for the local collab/RPC server when used.
actions!(
    client,
    [
...
pub fn init(client: &Arc<Client>, cx: &mut App) {
    // VIBEDEV: SignIn / SignOut on_action handlers removed (they invoked Zed
    // Cloud OAuth flow against zed.dev — wrong backend). Only Reconnect
    // survives, since it targets the local collab/RPC server when used.
