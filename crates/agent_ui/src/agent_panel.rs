                ...
                panel.update(cx, |panel, cx| {
                    ...
                    let initial_agent = match &thread_to_restore {
                        ...
                        None => serialized_panel
                            ...
                            .map(clamp)
                            .or(global_fallback)
                            // VIBEDEV: when nothing else is selected (first run /
                            // no last-used agent), default to the VibeDev custom
                            // agent (registered via agent_servers in default.json)
                            // so the panel opens to VibeDev instead of Zed's
                            // NativeAgent. Collab only supports NativeAgent → skip.
                            .or_else(|| {
                                (!is_via_collab).then(|| Agent::Custom {
...
impl AgentPanel {
            ...
            Rc::new(move |window, cx| {
                ...
                Some(ContextMenu::build(window, cx, |menu, _window, cx| {
                    menu.context(focus_handle.clone())
                        ...
                        // secondary. The native Zed agent is intentionally not
                        // surfaced (we ship no Zed AI sign-in).
                        .item(
                            ContextMenuEntry::new("VibeDev")
                                .when(
                                    !showing_terminal
                        ...
                        .map(|mut menu| {
                            ...
                            let agent_items = agent_server_store
                                .external_agents()
                                // VIBEDEV: VibeDev is promoted to the primary slot
                                // above, so don't also list it under "External Agents".
                                .filter(|agent_id| agent_id.0.as_ref() != "VibeDev")
                                .map(|agent_id| {
                            ...
                            if !agent_items.is_empty() {
                                // VIBEDEV: header reinstated — VibeDev is the
                                // primary agent above; this lists *other* agents.
    ...
    // VIBEDEV: rendering path (`render_trial_end_upsell`) was neutered with the Zed Cloud
    // removal; kept for upstream-rebase ease so the call site can be re-wired by patching here.
    #[allow(dead_code)]
    ...
    fn render_trial_end_upsell(
        ...
        // VIBEDEV: trial-end upsell removed — we don't ship Zed Pro / trial.
        // EndTrialUpsell still exists as a component but is never mounted.
        // The explicit type parameter pins the impl IntoElement return type
        // so the function still type-checks while always returning None.
        None::<gpui::Div>
    }
