...
fn parse_platform(output: &str) -> Result<RemotePlatform> {
    ...
    let os = match os {
        ...
        _ => anyhow::bail!(
            "Prebuilt remote servers are not yet available for {os:?}. See https://aitoken.bigopen.cn/docs/remote-development.html"
        ),
    };
        ...
        || arch.starts_with("aarch64")
    {
        ...
        anyhow::bail!(
            "Prebuilt remote servers are not yet available for {arch:?}. See https://aitoken.bigopen.cn/docs/remote-development.html"
        )
    };
