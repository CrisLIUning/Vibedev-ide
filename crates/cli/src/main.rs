...
#[command(
    ...
    before_help = "The VibeDev CLI binary.
This CLI is a separate binary that invokes VibeDev.

Examples:
    `vibedev`
          Simply opens VibeDev
    `vibedev --foreground`
          Runs in foreground (shows all logs)
    `vibedev path-to-your-project`
          Open your project in VibeDev
    `vibedev -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | vibedev -'"
)]
struct Args {
    ...
    reuse: bool,
    /// Open in existing VibeDev window
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/VibeDev`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\VibeDev`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/vibedev`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in VibeDev (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print VibeDev's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run VibeDev in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to VibeDev.app or the vibedev binary
    #[arg(long)]
    zed: Option<PathBuf>,
    /// Run VibeDev in dev-server mode
    #[arg(long)]
    dev_server_token: Option<String>,
    /// The username and WSL distribution to use when opening paths. If not specified,
    /// VibeDev will attempt to open the paths directly.
    ///
    ...
    wsl: Option<String>,
    /// Not supported in VibeDev CLI, only supported on VibeDev binary
    ...
    diff: Vec<String>,
    /// Uninstall VibeDev from user system
    #[cfg(all(
    ...
    /// by having VibeDev act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
...
fn run() -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://aitoken.bigopen.cn/docs/remote-development.html"
    );
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("VibeDev {} – {}", self.version(), self.path().display(),)
        }
