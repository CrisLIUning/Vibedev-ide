    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve VibeDev by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix VibeDev by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "VibeDev can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";
...
// VIBEDEV: replaces upstream's Zed-cloud "Zed Agent / Sign In" onboarding card.
// Clicking triggers OUR sub2api login (`vibedev_account::start_login`) rather
// than Zed's `client.sign_in_with_optional_connect` (that authenticates against
// zed.dev — the wrong backend). The sidecar returns a `login_url` we open in the
// browser; the VibeDev account panel polls and reflects the signed-in state.
fn render_vibedev_agent_button() -> impl IntoElement {
    AgentSetupButton::new("vibedev-agent-onboarding")
        .icon(
        ...
        .on_click(move |_, _window, cx| {
            telemetry::event!("Welcome VibeDev Sign In Clicked");
            let http = cx.http_client();
            cx.spawn(async move |cx| {
                if let Ok(login_url) = vibedev_account::start_login(http).await {
                    cx.update(|cx| cx.open_url(&login_url));
                }
            })
            .detach();
        })
